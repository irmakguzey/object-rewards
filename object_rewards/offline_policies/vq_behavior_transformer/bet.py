...
from third_person_man.models.vq_vae.vqvae import (
    VqVae,  # NOTE: This might give an error for nested import or smth
)

from .gpt import GPT
from .utils import MLP

# Taken from https://github.com/jayLEE0301/vq_bet_official/blob/main/vq_behavior_transformer/bet.py
...
class BehaviorTransformer(nn.Module):
    ...
    def __init__(
        ...
        if visual_input:
            ...
            self.resnet = torch.nn.Sequential(*(list(resnet.children())[:-1])).to(
                self.device
            )
    ...
    def _predict(
        ...
        # print("sampled_offsets.shape: {}".format(sampled_offsets.shape))
    ...
    def save_model(self, path: Path, model_type="best"):
        torch.save(self.state_dict(), path / f"cbet_model_{model_type}.pt")
        torch.save(self._gpt_model.state_dict(), path / f"gpt_model_{model_type}.pt")
        if hasattr(self, "resnet"):
            torch.save(self.resnet.state_dict(), path / f"resnet_{model_type}.pt")
            torch.save(
                self._resnet_header.state_dict(),
                path / f"resnet_header_{model_type}.pt",
            )
        torch.save(self.optimizer1.state_dict(), path / f"optimizer1_{model_type}.pt")
        torch.save(self.optimizer2.state_dict(), path / f"optimizer2_{model_type}.pt")

    def load_model(self, path: Path, model_type="best"):
        if (path / f"cbet_model_{model_type}.pt").exists():
            self.load_state_dict(torch.load(path / f"cbet_model_{model_type}.pt"))
        elif (path / f"gpt_model_{model_type}.pt").exists():
            self._gpt_model.load_state_dict(
                torch.load(path / f"gpt_model_{model_type}.pt")
            )
        elif (path / f"resnet_{model_type}.pt").exists():
            self.resnet.load_state_dict(torch.load(path / f"resnet_{model_type}.pt"))
        elif (path / f"optimizer1_{model_type}.pt").exists():
            self.optimizer1.load_state_dict(
                torch.load(path / f"optimizer1_{model_type}.pt")
            )
        elif (path / f"optimizer2_{model_type}.pt").exists():
            self.optimizer2.load_state_dict(
                torch.load(path / f"optimizer2_{model_type}.pt")
            )
        else:
