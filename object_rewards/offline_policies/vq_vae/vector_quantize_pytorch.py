...
def cdist(x, y):
    ...
    # print('in cdist - x.shape: {}, x2.shape: {}'.format(
    #     x.shape, x2.shape
    # ))
    y2 = reduce(y**2, "b n d -> b n", "sum")
    # print('in cdist - y.shape: {}, y2.shape: {}'.format(
    #     y.shape, y2.shape
    # ))
    xy = (
        einsum("b i d, b j d -> b i j", x, y) * -2
    )  # Supposed to be matrix multiplication
...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        # print('embed.shape: {}, num_codebooks: {}, codebook_size: {}, dim: {}'.format(
        #     embed.shape, num_codebooks, codebook_size, dim
        # ))
    ...
    def init_embed_(self, data, mask=None):
        # print(f'in init_embed_ - data(flatten).shape: {data.shape}, exists(mask): {exists(mask)}, self.initted: {self.initted}')
        ...
        # print('embed.shape: {}, cluster_size: {}'.format(embed.shape, cluster_size))

        embed_sum = embed * rearrange(cluster_size, "... -> ... 1")

        # print('embed_sum: {}'.format(embed_sum))
    ...
    def forward(self, x, sample_codebook_temp=None, mask=None, freeze_codebook=False):
        ...
        if needs_codebook_dim:
            x = rearrange(x, "... -> 1 ...")  # NOTE: You changed this part

        # print('x.shape in _codebook: {}'.format(x.shape))

        dtype = x.dtype
        flatten, ps = pack_one(x, "h * d")

        # print('flatten: {}, ps: {}'.format(
        #     flatten.shape, ps[0]
        # ))
        ...
        # print('flatten.shape: {}, embed.shape: {}'.format(flatten.shape, embed.shape))
        dist = -cdist(
            flatten, embed
        )  # This dist is calculated to get the nn from the codebook
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        # print('dim: {}, codebook_dim: {}, self.project_in: {}, project_out: {}'.format(
        #     dim, codebook_dim, self.project_in, self.project_out
        # ))
    ...
    def forward(
        ...
        x = self._codebook.transform_input(x)  # This stayed the same
